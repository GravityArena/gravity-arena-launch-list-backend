...
function isAuthorized(req) {
  ...
  const suppliedBearer = auth.toLowerCase().startsWith("bearer ")
    ? auth.slice(7).trim()
    : "";

  return Boolean(
    expected &&
      (suppliedHeader === expected || suppliedBearer === expected)
  );
}
...
async function sendWhatsAppText(to, text) {
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `WhatsApp reminder failed (${response.status}): ${detail.slice(0, 500)}`
    );
  }
}
...
function isConfirmedBookingReminder(reminder) {
  const bookingStatus = String(
    reminder?.booking_status ??
...
export default async function handler(req, res) {
  ...
  try {
    const due = await bookingRequest("/?action=reminders-due&limit=25", {
      method: "GET",
    });
    ...
    for (const reminder of reminders) {
      if (!isConfirmedBookingReminder(reminder)) {
        skipped += 1;

        console.warn("Booking reminder skipped", {
          reminderId: reminder?.reminder_id ?? null,
          bookingReference: reminder?.booking_reference ?? null,
          bookingStatus:
            reminder?.booking_status ??
            reminder?.bookingStatus ??
            null,
          reason: "Booking is not CONFIRMED or booking_status is missing.",
        });
